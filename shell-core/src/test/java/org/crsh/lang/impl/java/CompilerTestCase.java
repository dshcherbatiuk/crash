...
public class CompilerTestCase extends AbstractTestCase {
  ...
  public void testCompile() throws Exception {
    ...
    List<JavaClassFileObject> files =
        compiler.compile(
            "A",
    ...
    LoadingClassLoader loader =
        new LoadingClassLoader(Thread.currentThread().getContextClassLoader(), files);
            ...
            return new ClassLoader(Thread.currentThread().getContextClassLoader()) {
              ...
              protected Class<?> findClass(String name) throws ClassNotFoundException {
                if (name.equals("foo.A")) {
                  if (aClass == null) {
                    // Normally we should use the bytes from the nested crash.jar but it's too
                    // difficult
              ...
              protected Enumeration<URL> findResources(String name) throws IOException {
                if ("META-INF/MANIFEST.MF".equals(name)) {
                  ...
                  URL u2 =
                      new URL(
                          "jar:"
                              + ("jar:" + tmp.toURI().toURL() + "!/WEB-INF/lib/crash.jar")

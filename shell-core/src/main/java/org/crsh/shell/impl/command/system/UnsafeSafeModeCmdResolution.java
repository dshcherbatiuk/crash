...
public class UnsafeSafeModeCmdResolution {
  public abstract static class UnSafeCommand extends BaseCommand {
    ...
    public void main(InvocationContext<Object> context) throws Exception {
      context.provide(
          new LabelElement(
                  "Unsafe system/script command ["
                      + getCommandName()
                      + "] is not available with shell in safe mode.")
              .style(Style.style(Decoration.bold, Color.red)));
    }
  }

  // Adds safe dummy handlers for unsafe commands in the safe mode
  static void addSafeHandlers(
      HashMap<String, Class<? extends BaseCommand>> safeCommandsHandlers,
      boolean exitAllowed,
      boolean manAllowed) {
    if (!exitAllowed) {
  ...
  static void addSafeHandlerForMan(
      HashMap<String, Class<? extends BaseCommand>> safeCommandsHandlers) {

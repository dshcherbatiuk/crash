...
class EditorAction {
  ...
  public abstract static class History extends EditorAction {
    ...
    void perform(Editor editor, EditorBuffer buffer) throws IOException {
      ...
      if (nextHistoryCursor >= -1 && nextHistoryCursor < editor.history.size()) {
        String s =
            nextHistoryCursor == -1 ? editor.historyBuffer : editor.history.get(nextHistoryCursor);
  ...
  static class MovePrevWord extends EditorAction {
    ...
    void perform(Editor editor, EditorBuffer buffer) throws IOException {
      ...
      while (pos > 0) {
        ...
        if ((atBeginning && Character.isLetterOrDigit(c))
            || (!atBeginning && !Character.isLetterOrDigit(c))) {
          break;
        } else {
          pos--;
        }
    ...
    void perform(Editor editor, EditorBuffer buffer) throws IOException {
      ...
      while (true) {
        ...
        if (look < to) {
          ...
          if ((at != At.BEGINNING && Character.isLetterOrDigit(c))
              || (at == At.BEGINNING && !Character.isLetterOrDigit(c))) {
            break;
          } else {
            pos++;
          }
  ...
  abstract static class Copy extends EditorAction {
              ...
              if (pos < buffer.getSize()) {
                if (buffer.moveLeft()) {
                  ...
                  if (editor.console.getMode() == Mode.VI_MOVE
                      && buffer.getCursor() > editor.getCursorBound()) {
                    buffer.moveLeft();
                  }
                }
              } else {

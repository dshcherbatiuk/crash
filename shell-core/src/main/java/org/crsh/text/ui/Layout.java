...
public abstract class Layout {
   ...
   *
   * <ul>
   *   <li>the sum of the returned array elements must be equals to the <code>totalLength</code>
   *       argument
   *   <li>a cell length should never be lesser than the provided minimum length
   * </ul>
   *
   * The returned array is the list of lengths from left to right, the array size may be less than
   * the number of cells (i.e the size of the <code>actualLengths</code> and <code>minLengths</code>
   * arguments). Missing cells are just be discarded and not part of the resulting layout. Array
   * should contain only positive values, any zero length cell should be discarded. When cells must
   * be discarded it must begin with the tail of the list, i.e it is not allowed to discard a cell
   * that does not have a successor.
   *
